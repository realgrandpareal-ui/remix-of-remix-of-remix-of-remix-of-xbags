  ...
  return (
    <div className="container mx-auto max-w-3xl px-4 py-6">
      ...
      <div className="relative z-10 px-4">
        <div className="h-20 w-20 rounded-full bg-card border-4 border-background flex items-center justify-center text-2xl font-bold text-gradient">BF</div>
        <div className="mt-3 flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div>
            <h1 className="text-xl font-bold">bags.fun User</h1>
            <p className="text-sm text-muted-foreground">@{username || "creator"}</p>
            <p className="text-sm text-foreground mt-2 max-w-md">Digital artist & Web3 builder. Creating on bags.fun 🎨</p>
          ...
          <Button variant="outline" size="sm" className="gap-2 border-border self-start hover:border-primary"><Edit className="h-3.5 w-3.5" /> Edit Profile</Button>
        </div>
        <div className="flex gap-6 mt-6">
          {[{ value: "1,234", label: "Posts" }, { value: "45.2K", label: "Followers" }, { value: "892", label: "Following" }].map((s) => (
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          ...
          <Badge className="bg-info/10 text-info border-info/20">Top 100</Badge>
